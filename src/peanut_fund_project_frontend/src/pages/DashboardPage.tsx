...
export const DashboardPage: React.FC<DashboardPageProps> = () => {
  ...
  async function handleGreet() {
    ...
    setIsGreeting(true);
    try {
      const result = await backend_actor.greet(name);
  ...
  const fetchData = async () => {
    try {
      setRefreshing(true);
      const portfolioResult = await backend_actor.getPortfolio();
  ...
  return (
    <div className="container mx-auto px-4 py-8">
        ...
        <h2 className="text-xl font-bold">ICP Backend Communication Example</h2>
        <div className="flex items-center gap-2 mt-2">
            ...
            />
            <button onClick={handleGreet} disabled={isGreeting} className="px-4 py-2 bg-blue-500 text-white rounded disabled:bg-blue-300">
              {isGreeting ? 'Greeting...' : 'Greet'}
            </button>
        </div>
        {greeting && <p className="mt-4 text-lg">Backend says: <span className="font-bold">{greeting}</span></p>}
      </div>
